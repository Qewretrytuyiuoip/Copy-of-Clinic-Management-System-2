...
const SubManagerFormModal: React.FC<SubManagerFormModalProps> = ({ subManager, onSave, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isEditMode && !formData.password) {
            alert('كلمة المرور مطلوبة للمدير الفرعي الجديد.');
            return;
        }
        setIsSaving(true);
...
const AdminsTab: React.FC<{ refreshTrigger: number }> = ({ refreshTrigger }) => {
    ...
    const allManagers = [...admins, ...subManagers];

    // Card component for user
    // FIX: Changed component props typing to React.FC to correctly handle React-specific props like 'key'.
    const UserCard: React.FC<{ user: User }> = ({ user }) => (
        <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 flex flex-col justify-between transition-shadow hover:shadow-lg">
            <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{user.email}</p>
                 <span className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">
                    {user.role === UserRole.Admin ? 'مدير' : 'مدير فرعي'}
                </span>
            </div>
            {user.role === UserRole.SubManager && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600 flex items-center justify-end space-x-2">
                    <button onClick={() => setEditingSubManager(user)} className="flex items-center text-blue-600 dark:text-blue-400 hover:text-blue-800 p-1 rounded hover:bg-blue-100 dark:hover:bg-blue-900/40 text-sm"><PencilIcon className="h-4 w-4" /><span>تعديل</span></button>
                    {loggedInUser && user.id !== loggedInUser.id && (
                        <button onClick={() => setDeletingSubManager(user)} className="flex items-center text-red-600 dark:text-red-400 hover:text-red-800 p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/40 text-sm"><TrashIcon className="h-4 w-4" /><span>حذف</span></button>
                    )}
                </div>
            )}
        </div>
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100">قائمة المدراء</h2>
            ...
            {isAdding && <SubManagerFormModal onClose={() => setIsAdding(false)} onSave={handleCreate} />}
            
            {editingSubManager && <SubManagerFormModal subManager={editingSubManager} onClose={() => setEditingSubManager(null)} onSave={handleUpdate} />}
            
            {deletingSubManager && (
                <ConfirmDeleteModal
                    title="حذف مدير فرعي"
                    message={`هل أنت متأكد من رغبتك في حذف ${deletingSubManager.name}؟ لا يمكن التراجع عن هذا الإجراء.`}
                    onConfirm={confirmDelete}
                    onCancel={() => setDeletingSubManager(null)}
                />
            )}
        </div>
    );
