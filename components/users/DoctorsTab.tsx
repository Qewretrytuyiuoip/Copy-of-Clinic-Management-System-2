...
import { PlusIcon, PencilIcon, TrashIcon, XIcon, UserGroupIcon, UserCircleIcon } from '../../components/Icons';
                ...
                {loading ? <CenteredLoadingSpinner /> : fetchError ? (
                     ...
                     doctors.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {doctors.map(doc => (
                                <div key={doc.id} className="relative group overflow-hidden rounded-3xl bg-white/40 dark:bg-slate-800/40 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                                    ...
                                    <div className="relative z-10 p-5 flex flex-col h-full justify-between">
                                        ...
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2 rounded-xl bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">
                                                    <UserCircleIcon className="h-8 w-8" />
                                                </div>
                                                <div>
                                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white line-clamp-1" title={doc.name}>{doc.name}</h3>
                                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1" title={doc.email}>{doc.email}</p>
                                                </div>
                                            </div>
                                            {doc.is_diagnosis_doctor && (
                                                <span className="px-2 py-0.5 bg-cyan-100 text-cyan-800 dark:bg-cyan-900 dark:text-cyan-300 text-[10px] font-bold rounded-full">
                                                    تشخيص
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <div className="mb-4">
                                            <p className="text-sm text-gray-600 dark:text-gray-300">
                                                <span className="font-semibold text-primary dark:text-primary-400">التخصص: </span>
                                                {doc.specialty || 'غير محدد'}
                                            </p>
                                        </div>
                                        ...
                                        <div className="pt-4 border-t border-gray-100 dark:border-white/10 flex justify-end gap-2">
                                            <button 
                                                ...
                                                className="flex items-center justify-center p-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-slate-700 dark:text-blue-400 dark:hover:bg-slate-600 transition-colors shadow-sm"
                                                title="تعديل"
                                            >
                                            ...
                                            <button 
                                                ...
                                                className="flex items-center justify-center p-2 rounded-xl bg-red-50 text-red-600 hover:bg-red-100 dark:bg-slate-700 dark:text-red-400 dark:hover:bg-slate-600 transition-colors shadow-sm"
                                                title="حذف"
                                            >
